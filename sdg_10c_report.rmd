...
The analysis presented in this report addresses an underrepresented understanding of research and dialogue on global remittance corridors and their effect on the global economic climate. Through following an anti naturalist methodology, our analysis does not seek to explain the social human phenomenon of migrant capital exchange simply through the causal dependence of social reality representation, nor does it wish to exploit the privacy concerns of highly aggregated data which in its purest form is tremendously sensitive, instead, the analysis aims to build a dialogue around climate vulnerable countries and their digital manifestation in open data from intergovernmental organizations, and the impacts this has on the Sustainable Development Goals policy framework. 

As previously stated, there are two components to SDG 10.C, the former being the reduction of global remittance cost to 3% and the latter being the reduction of average cost to 5%. To monitor this economic science, the World Bank has assembled the largest remittance database, which includes 131 unique countries and their remittance corridor rates based on the World Banks “Mystery shopping” methodology [REF BLOG POST].

The two research questions guiding this analysis are:

- What is the current global state of SDG 10.C?
- How are climate vulnerable countries represented in the existing data?


## Library Installation

To conduct the analysis, the following packages were utilised for data cleaning,transformation and visualization: 
```{r eval=FALSE}
####
####
...
The raw [data](https://databank.worldbank.org/source/remittance-prices-worldwide-(corridors)) provided by the World Bank is by no means clean/tidy data, and any form of analysis cannot be complete without tidying this data into a machine readable format. The statistical results of a data quality diagnosis can be found in our analysis [repository](https://github.com/natesheehan/sdg_10C/blob/main/data/wb_data_quality.csv). 

The data is cleaned by renaming column names and countries (so the geocoder API can function), converting characters into numeric, averaging time series data and producing a long data format. 
